...
def image():
    if request.method == 'POST':
        ...
        IMG_SIZE = 50
        LR = 1e-3
        MODEL_NAME = 'pancreatic-{}-{}.model'.format(LR, '2conv-basic')
    ##    MODEL_NAME='keras_model.h5'
        ...
        for num, data in enumerate(verify_data):
            ...
            if np.argmax(model_out) == 0:
                ...
                accuracy="The predicted image of the Normal is with a accuracy of {}%".format(model_out[0]*100)
                
                
            elif np.argmax(model_out) == 1:
                str_label = "Pancreatic"
                print("The predicted image of the Pancreatic is with a accuracy of {} %".format(model_out[1]*100))
                accuracy="The predicted image of the Pancreatic is with a accuracy of {}%".format(model_out[1]*100)
            A=float(model_out[0])
            B=float(model_out[1])
            
           
            
            dic={'Normal':A,'Pancreatic':B}
            algm = list(dic.keys()) 
            ...
            plt.title("Accuracy Comparision between Pancreatic Detection")
